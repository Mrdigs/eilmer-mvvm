// DUE TO BE DELETED WHEN <Bind> IS FINISHED
...
function Binding({ vm, converter, children, ...props }) {
  if (Array.isArray(children)) {
      ...
      .forEach((property) => {
        if (property.isExpression) {
          ...
          if (typeof binding !== "undefined") {
            ...
            console.warn(
              property.name,
              'value "' + property.value + '" does not exist'
            );
          }
        } else {
          ...
          if (
            property.isEvent ||
            currentValue instanceof Command ||
            typeof currentValue === "function"
          ) {
            // Command Binding
            // console.log('Binding command: ', property.name, property.value)
            const [execute, canExecute, context] = binder.useCommand(
              property.value,
              property.converter
            );
            ...
            // Property Binding
            const [value, setValue, context] = binder.useBinding(
              property.value,
    ...
    return <BoundChild {...childProps}>{children}</BoundChild>;
  }
}
...
function BoundChild(props) {
  ...
  const { propertyBindings, children, ...childProps } =
  ...
  if (propertyBindings) {
    Object.values(propertyBindings).forEach((binding) => {
      ...
      if (binding)
        binding.getContext().setComponentPropertiesHandler = (
          converterProps
        ) => {
          setSavedProps((savedProps) => ({
...
class BindProperty {
  ...
  constructor(name, value) {
    ...
    this.isReference =
      this.isReference || (this.isExpression && value.includes("@"));
  }
