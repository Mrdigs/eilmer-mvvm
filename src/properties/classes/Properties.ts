...
export default class Properties {
  static addPropertyChangeListener(
    target: object,
    propertyName: string,
    listener: Listener
  ) {
    ...
    const descriptor: PropertyDescriptor | any =
      getPropertyDescriptor(object, property) || defaultDescriptor;
    ...
    if (!descriptor.set?.listeners) {
      ...
      descriptor.set = function (v: any, notify: boolean) {
        const old = accessor.get(this),
          result = accessor.set(this, v);
      ...
      if (descriptor !== defaultDescriptor) {
        ...
        console.warn(
          `[Eilmer] Binding to an non-existent property ${property}. This isn't a problem, but may not be intentional`
        );
      }
    ...
    if (object[property]?.isObservable?.call()) {
      ...
      object[property].subscribe(
        descriptor.set.bind(object, object[property], true)
      );
    }
    return () => {
  ...
  static removePropertyChangeListener(
    target: object,
    propertyName: string,
    listener: Listener
  ) {
...
function notifyPropertyChangedInternal(
  targetObject: object,
  propertyName: string,
  regardless: boolean
) {
  const [object, property] = getTargetAndPropertyName(
    targetObject,
    propertyName
  );
  // If the descriptor doesn't exist then we shouldn't error
  const descriptor = Object.getOwnPropertyDescriptor(object, property);
  const set = descriptor.set as unknown as {
    listeners: Listener[];
    rev: number;
  };
  if (set?.listeners) {
...
function getTargetAndPropertyName(
  targetObject: object,
  propertyName: string
): [object, string] {
