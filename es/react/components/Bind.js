...
export default function Bind({
  ...
  */
  ...
  return /*#__PURE__*/React.createElement(Bindings, boundProps, children);
}
...
function Bindings({
  ...
  useEffect(() => {
    const unbinds = Object.entries(bindings).map(([property, binding]) => {
      return binding.bind(value => setProps(props => ({ ...props,
        [property]: value
      })));
    });
...
function propsToBindings(props) {
  ...
  Object.keys(props).sort().forEach(propKey => {
    ...
    if (propKey.startsWith("$")) {
      ...
      const expression = '[' + propValue.substring(6, propValue.lastIndexOf(')')) + ']';
      const variableResolver = new ObjectVariableResolver(expressionContext);
      const result = new Expression(expression).evaluate(variableResolver); // TODO: I need support for a converter, too
