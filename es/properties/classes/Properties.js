...
export default class Properties {
  static addPropertyChangeListener(target, propertyName, listener) {
    ...
    const descriptor = getPropertyDescriptor(object, property) || defaultDescriptor; // SHOULD I ALWAYS USE OBJECT CREATE IN, SAY, BINDER?
    ...
    if (!descriptor.set?.listeners) {
      ...
      descriptor.set = (v, notify) => {
        const old = accessor.get(),
              result = accessor.set(v);
        ...
        if (v !== old || notify) {
          ...
          notifyPropertyChangedInternal(object, property, notify); // notifyPropertyChanged(object, property, v, old)
        }
...
function notifyPropertyChangedInternal(target, propertyName, regardless) {
  const [object, property] = getTargetAndPropertyName(target, propertyName); // If the descriptor doesn't exist then we shouldn't error

  const descriptor = Object.getOwnPropertyDescriptor(object, property);

  if (descriptor?.set?.listeners) {
    const value = object[property],
          rev = ++descriptor.set.rev;
